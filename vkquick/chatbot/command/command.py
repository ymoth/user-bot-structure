...
class Command(HandlerMixin[Handler]):
    ...
    async def handle_message(self, ctx: NewMessage) -> None:
        ...
        if user is not None:
            self.prefixes = (await user.get_settings()).prefixes
        ...
        if is_routing_matched:
            ...
            if arguments is not None:
                ...
                if passed_filter:
                    from src.utils import process_after_message_handle_is_complete
                    await asyncio.gather(
                        process_after_message_handle_is_complete(context=ctx),
                        self._call_handler(ctx, arguments)
                    )
                return
    ...
    async def _call_handler(self, ctx: NewMessage, arguments: dict) -> None:
        logger.opt(colors=True).success(
            **format_mapping(
                "Send command: <m>{com_name}</m><w>({params})</w>",
                "<c>{key}</c>=<y>{value!r}</y>",
                arguments,
            ),
