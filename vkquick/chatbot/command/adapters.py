    ...
    *,
    ...
    if arg_annotation is int:
        ...
        if arg_kind == inspect.Parameter.KEYWORD_ONLY:
            ...
            return WordCutter()
    elif arg_annotation.__dict__.get("__module__") == "src.new_line_adapter":
        cutter = _resolve_cutter(
            arg_name=arg_name,
            arg_annotation=arg_annotation.cutter,
            arg_settings=arg_settings,
            arg_kind=arg_kind
        )
        if hasattr(cutter, "_pattern"):
            cutter._pattern = re.compile(f"{cutter._pattern.pattern}\n?")
        return cutter
